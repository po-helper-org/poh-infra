...
- **Два обработчика на одном репозитории.** Если на нём же стоит прод-App,
  ...
  прогона бери репозиторий, на который прод-App не установлен. То же и для ревью:
  два PR-Agent на одном PR не столько дублируют комментарии, сколько вдвое быстрее
  выбирают лимит общего ключа модели.
- **PR открылся, а кода в нём нет.** Каталог задачи не передан раннеру: он
  работает от uid 10001, а воркер готовит каталог от root. Агент не падает —
  пишет в `/tmp` и докладывает об успехе. Проверка: `ls -ld` каталога задачи в
  томе `poh-dev-workspace` должен показывать владельца 10001.
- **Выкладка посреди прогона.** Правка РЕШЕНИЯ воркфлоу без `workflow.patched`
  роняет идущие прогоны по недетерминизму, и Issue перестаёт отвечать на сигналы
  вовсе; долгая активность умирает вместе с воркером по heartbeat; контейнер
  агента переживает своего запускателя. Подробности и проверки — в
  [`STATUS.md`](STATUS.md).
