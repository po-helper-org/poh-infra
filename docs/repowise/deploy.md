...
Тем же способом, что у Temporal UI контура: имя в зоне `traefik.me` с адресом
сервера прямо в имени. **DNS настраивать не нужно вовсе** — `traefik.me`
отвечает на любое `<что-угодно>-<a>-<b>-<c>-<d>.traefik.me` адресом `a.b.c.d`.

```bash
REPOWISE_PUBLIC_HOST=poh-repowise-<ip-через-дефисы>.traefik.me
```

Маршрут заводят метки Traefik в `docker-compose.yml`; панель Dokploy для этого
не нужна — Traefik берёт правило `Host(...)` из меток контейнера.

**Только http.** Сертификат на `traefik.me` не выписывается: Let's Encrypt
отдаёт `429` — лимит 50 сертификатов за 168 часов на весь домен (он не в PSL),
и окно выбирают чужие стенды за минуты. Роутер на `websecure` в метках описан и
готов, но заработает лишь со своим доменом и A-записью на сервер.

Следствие для безопасности: пароль админки ходит по http в base64. Для стенда
это принято осознанно, для боевого контура — нет.

## Шаг 2б. Учётка веб-админки
...
**На сервере контура её НЕ запускают.** Там свободной памяти около 200 МБ при
выключенном swap и живут чужие боевые сервисы; всплеск индексации выбирает
память хоста, и OOM-killer выбирает жертву сам. Индекс строят на машине
разработки и привозят готовым — 9 репозиториев `contour` дают около 67 МБ,
архивом 21 МБ.
...
```bash
# 1. Упаковать (COPYFILE_DISABLE — иначе macOS доложит ресурс-форки ._*)
cd <каталог с рабочими копиями>
COPYFILE_DISABLE=1 tar czf index.tgz contour

# 2. Привезти и распаковать в том
scp index.tgz <сервер>:/opt/poh-repowise/
ssh <сервер> 'docker volume create poh-repowise_workspaces'
ssh <сервер> 'docker run --rm -v poh-repowise_workspaces:/dst -v /opt/poh-repowise:/src \
  alpine tar xzf /src/index.tgz -C /dst'
```

Том объявлен в compose как `external` именно поэтому: его заводят и наполняют
до первого `up`.

**Эмбеддер после переноса.** В конфигурации репозиториев остаётся то значение,
с которым строили индекс. Если ключа на сервере нет — переключить на `mock`,
иначе семантический запрос не выполнится вовсе: эмбеддится не только страница,
но и сам вопрос.

```bash
docker run --rm -v poh-repowise_workspaces:/w alpine sh -c \
  'sed -i "s/^embedder: .*/embedder: mock/" /w/contour/*/.repowise/config.yaml'
```

## Шаг 4б. Первичная индексация на месте (только вне контура)
...
Каждый сервис объявлен с `mem_limit`. Это не гигиена, а условие соседства:
без лимита всплеск выбирает память хоста и жертву выбирает ядро — ей может
оказаться чужой прод. С лимитом умирает наш контейнер, и это чинится
рестартом и видно в логах.

Замерено на сервере после выкладки:

| Сервис | Потребление | Лимит |
|---|---|---|
| `repowise-contour-mcp` | 227 МБ | 512 МБ |
| `repowise-proxy` | 52 МБ | 128 МБ |
| `indexer` (в покое) | 95 МБ | 768 МБ |

Поднимать лимит по факту нехватки, а не впрок: запас, взятый заранее, здесь
отнимается у соседа.
